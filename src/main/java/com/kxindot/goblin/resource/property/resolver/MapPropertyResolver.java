...
public class MapPropertyResolver<V> extends AbstractPropertyResolver<Map<String, V>> {
	...
	@Override
	protected Map<String, V> resolve(String name, Properties properties) {
		...
		if (!ignoreNotFound && map.isEmpty()) {
			threx(PropertyResolveException::new, "未找到%s对应的配置信息!", name);
		}
		return map;
	}

	Map<String, V> getInstance() {
		int mod = type.getModifiers();
		return Modifier.isAbstract(mod) || Modifier.isInterface(mod) ? newLinkedHashMap() : newInstance(type);
	}
